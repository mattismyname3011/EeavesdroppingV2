...
export default function Computer2Page() {
  ...
  useEffect(() => {
    ...
    socketInstance.on('message-to-computer2', (msg: Message) => {
  ...
  const sendMessage = () => {
    if (socket && inputMessage.trim()) {
      socket.emit('message-from-computer2', { content: inputMessage.trim() })
  ...
  return (
    <div className="min-h-screen flex flex-col bg-gradient-to-br from-green-50 to-slate-50">
                  ...
                  {messages.length === 0 ? (
                    ...
                    messages.map((msg) => (
                      <div
                        key={msg.id}
                        className={`flex ${
                          msg.sender === 'computer2' ? 'justify-end' : 'justify-start'
                        }`}
                      >
                        <div
                          className={`max-w-xs md:max-w-md px-4 py-2 rounded-lg ${
                            msg.sender === 'computer2'
                        ...
                        >
                          <div className="flex items-center gap-2 mb-1">
                            <span className="text-xs font-medium">
                              {msg.sender === 'computer2' ? 'You' : 'Computer 1'}
                            </span>
                          </div>
      ...
      <footer className="mt-auto py-4 bg-slate-900 text-slate-400 text-xs text-center">
        <p>Computer 2 - Eavesdropping Demo</p>
      </footer>
    </div>
  )
}
